...
export default function LandingPage() {
  ...
  const plans = [
    {
      ...
      buttonColor: 'bg-textmain hover:bg-primarydark'
    },
    {
      name: 'Intermediário',
      price: 'R$ 149',
    ...
    },
    {
      ...
      color: 'border-alert',
      buttonColor: 'bg-alert hover:bg-primarydark'
    }
  ];
  ...
  return (
    <div className="min-h-screen bg-white text-textmain">
      {/* Header */}
      <header className="fixed top-0 w-full bg-white/90 backdrop-blur-sm z-50 border-b border-fog">
        ...
        {/* Mobile Menu */}
        {isMenuOpen && (
          <div className="md:hidden bg-white border-t border-fog">
            <div className="px-4 py-2 space-y-1">
              <a href="#como-funciona" className="block py-2 hover:text-accent">Como Funciona</a>
              <a href="#planos" className="block py-2 hover:text-accent">Planos</a>
              <a href="#depoimentos" className="block py-2 hover:text-accent">Depoimentos</a>
              <a href="#contato" className="block py-2 hover:text-accent">Contato</a>
            </div>
          </div>
        )}
      </header>
      ...
      {/* Hero Section */}
      <section className="pt-24 pb-16 bg-gradient-to-br from-fog to-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <h1 className="text-4xl md:text-6xl font-bold mb-6">
              Proteja quem você
              <span className="text-accent"> ama</span> com
              <span className="text-accent"> segurança inteligente</span>
            </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center items-center mb-12">
              <a
                href="#contato"
                className="bg-accent text-white px-8 py-4 rounded-lg font-semibold hover:bg-primary transition-colors flex items-center space-x-2"
              >
              ...
              <a
                href="#como-funciona"
                className="text-accent border-2 border-accent px-8 py-4 rounded-lg font-semibold hover:bg-fog transition-colors"
              >
                Como funciona?
              </a>
            </div>
            <div className="max-w-4xl mx-auto">
              {/* Imagem local servida a partir de /public */}
              <img
                src="/sentinela-hero.jpg"
      ...
      <section id="como-funciona" className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-4 gap-8">
            {[
              { step: '01', icon: MessageCircle, title: 'Mensagem via WhatsApp', description: 'Enviamos uma mensagem automática de checagem no horário programado' },
              { step: '02', icon: Clock,         title: 'Aguardamos resposta',   description: 'Damos tempo para a pessoa responder confirmando que está bem' },
              { step: '03', icon: Phone,         title: 'Escalamento automático', description: 'Se não houver resposta, ligamos e contatamos familiares imediatamente' },
              { step: '04', icon: Users,         title: 'Equipe presencial',      description: 'Em casos críticos, enviamos nossa equipe treinada ao local' }
            ].map((item, index) => (
              <div key={index} className="text-center">
                <div className="bg-primary/10 rounded-full w-20 h-20 mx-auto mb-6 flex items-center justify-center relative">
                  <item.icon className="h-10 w-10 text-primary" />
                  <span className="absolute -top-2 -right-2 bg-primary text-white rounded-full w-8 h-8 flex items-center justify-center text-sm font-bold">
                    {item.step}
                  </span>
                </div>
      ...
      {/* Planos */}
      <section id="planos" className="py-16 bg-fog">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold mb-4">
          ...
          <div className="grid md:grid-cols-4 gap-8">
            {plans.map((plan, index) => (
              <div
                key={index}
                className={`bg-white rounded-xl shadow-lg p-6 border ${plan.color} relative ${plan.popular ? 'transform scale-105' : ''}`}
              >
                {plan.popular && (
            ...
            {testimonials.map((testimonial, index) => (
              <div key={index} className="bg-fog rounded-xl p-6">
                <div className="flex items-center mb-4">
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} className="h-5 w-5 text-alert fill-current" />
                  ))}
                </div>
      ...
      {/* Formulário de Lead */}
      <section id="contato" className="py-16 bg-fog">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <button
                type="submit"
                className="w-full bg-accent text-white py-4 px-8 rounded-lg font-semibold hover:bg-primary transition-colors flex items-center justify-center space-x-2"
              >
          ...
          <div className="grid md:grid-cols-4 gap-8">
            <div>
              ...
              <div className="flex space-x-4">
                <Facebook className="h-6 w-6 text-white/70 hover:text-white cursor-pointer" />
                <Instagram className="h-6 w-6 text-white/70 hover:text-white cursor-pointer" />
                <Linkedin className="h-6 w-6 text-white/70 hover:text-white cursor-pointer" />
              </div>
            </div>
      ...
      <a
        ...
        className="fixed bottom-6 right-6 bg-success text-white p-4 rounded-full shadow-lg hover:bg-primary transition-colors z-40 hover:scale-110 transform"
      >
